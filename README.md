...
**Why Brent–WTI, briefly:** Brent (seaborne, globally traded) and WTI (inland, Cushing-delivered) price different things despite tracking closely, so their spread compresses information about market integration — storage, pipeline capacity, export infrastructure, freight — not just a statistical curiosity. See [`MARKET_CONTEXT.md`](MARKET_CONTEXT.md) for the full economic framing, including two concrete historical episodes and what the statistics here can't tell you on their own.
...
* Frozen-parameter, walk-forward out-of-sample evaluation across three periods (2019–2026): Sharpe of `1.01`, `1.07`, and `1.21` net of transaction costs for development, validation, and final-test respectively, each with a bootstrap 95% confidence interval (see "Results" for the full table).
...
| `spread_model.py`          | Shared model logic: Kalman hedge ratio, z-score, regime filter, risk scaling, position logic, and performance metrics (including a standalone Sharpe/Sortino/return function reused by `bootstrap_ci.py` and `costs.py`'s bid-ask layer) — imported identically by every script below, so none of them can silently disagree about what "the spread" is |
...
| `costs.py`                 | Two cost analyses: a flat-bps transaction-cost stress test (low / base / stress scenarios, base fixed to match `config.cost_per_turnover`), and a separate bid-ask spread layer applied on top of the committed OOS numbers (`results/oos_daily_returns.csv`) using each contract's real exchange tick size — see "Results" |
| `bootstrap_ci.py`          | Stationary block bootstrap (10,000 resamples) for 95% confidence intervals on Sharpe, Sortino, and total return, per OOS period — reads `results/oos_daily_returns.csv`, no network access, no recomputation                                                                  |
...
* **Regime filter:** new entries are blocked whenever recent spread volatility sits in the top 30% of its trailing 100-day range — a volatility-gating rule, not a full regime-detection model. This only gates entries: a position already open when the regime turns unfavorable is **not** force-closed — it still exits normally via the z-score threshold below. "Trading is switched off" describes new entries, not a flat book.
...
Every rolling statistic feeding a signal — the z-score's rolling mean/std, and risk_scale's EWMA vol estimate — uses `.shift(1)`, so today's threshold comparisons never use today's own value in the window. The z-score and position themselves are allowed to use today's spread and today's signal (that's what "the signal is knowable at today's close" means) — what must never happen, and doesn't, is using today's information to affect what gets PAID today: the PnL step explicitly lags both position and beta by one day (`pos_lag`, `beta_lag`) against today's return, so a decision made from today's close only starts earning or losing money from tomorrow. (`risk_scale` is shifted inside `compute_risk_scale`; `compute_performance` applies `position.shift(1)` and `df["beta"].shift(1)` — see `spread_model.py` for exactly where each shift lives.)
...
python bootstrap_ci.py          # 95% CI on Sharpe/return per period -> results/bootstrap_ci.csv (no network needed)
python costs.py                 # bid-ask spread cost layer on the committed OOS numbers -> results/bid_ask_layer.csv (no network needed)
...
| Bid-ask spread layer (`costs.run_bid_ask_layer`) | `1 / 2 / 3` ticks, tick = `$0.01/bbl` | Tick size is verified from CL's and BZ's own exchange contract specs, not guessed. Spread *width* in ticks is not verified (yfinance has no bid/ask history) — tested as a range rather than asserted as one number. Layered on top of the 5 bps cost above, not instead of it. |
...
**Bootstrap confidence intervals** (stationary block bootstrap, mean block length 10 trading days, 10,000 resamples — see `bootstrap_ci.py` for why this method and this block length; full output in `results/bootstrap_ci.csv`):

| Period      | Sharpe (95% CI)        | Sortino (95% CI)       | Return (95% CI)          |
| ----------- | ----------------------- | ----------------------- | -------------------------- |
| Development | 1.009 `[0.14, 1.74]`    | 1.198 `[0.14, 2.62]`    | 9.77% `[1.01%, 19.96%]`    |
| Validation  | 1.070 `[-0.01, 1.97]`   | 1.385 `[-0.01, 3.12]`   | 7.79% `[-0.15%, 16.34%]`   |
| Final test  | 1.207 `[0.30, 2.11]`    | 0.901 `[0.17, 3.66]`    | 12.56% `[3.06%, 22.57%]`   |

Worth being direct about, not glossing over: validation's Sharpe and Sortino intervals both come within a hair of zero on the low end. At 95% confidence, this period's data alone cannot fully rule out "no real edge" — the point estimate is positive, but the interval is wide enough that this isn't a strong individual result. Development and final-test both stay clear of zero with more room. Confidence intervals this wide, on periods this length, are the expected shape of the uncertainty here — not a defect of the bootstrap.

**Bid-ask spread layer** (stylized, layered on top of the numbers above, not instead of them — see `costs.py` for what's verified vs. assumed): using each contract's real `$0.01`/barrel exchange tick as the unit and testing 1–3 assumed ticks of spread width, Sharpe drops to roughly `0.42–0.82` (development), `0.60–0.92` (validation), and `0.57–1.00` (final-test) depending on the assumed width. Stays positive across the full tested range, but this is a genuine, non-trivial bite — a meaningfully bigger drag than the flat 5 bps assumption used everywhere else in this backtest implies on its own. Full output in `results/bid_ask_layer.csv`.
...
* "Still to fill in: block bootstrap CI on Sharpe (planned)" → implemented in `bootstrap_ci.py` (stationary block bootstrap, 10,000 resamples per period); results in "Results" above and `results/bootstrap_ci.csv`.
* The cost stress test only ever varied a single flat bps assumption, with no cost source tied to the actual contracts traded → added `costs.run_bid_ask_layer`, a separate cost layer using CL's and BZ's real exchange-verified tick size, layered on top of (not instead of) the existing flat-bps assumption; results in "Results" above and `results/bid_ask_layer.csv`.
* The Sharpe/Sortino/return formula lived only inside `compute_performance`, with no single place for other analyses to reuse it → factored out into `spread_model.performance_metrics_from_log_returns` before `bootstrap_ci.py` and the bid-ask layer needed the same formula a second and third time, so both call the same function instead of risking a silently-drifting copy (verified: the refactor reproduces `results/oos_results.csv`'s existing numbers to float precision).
* No economic framing anywhere in the repo — every diagnostic was statistical (Sharpe, ADF, beta), with nothing on why Brent and WTI relate the way they do, or what could make the historical relationship stop holding → added `MARKET_CONTEXT.md`, kept separate from this file rather than inserted here, since its content and register (economic reasoning, not code mechanics) don't fit this README's format.
* The regime filter description said "trading is switched off" without qualification → clarified: it blocks new entries only; an already-open position is not force-closed when the regime later turns unfavorable, it still exits normally via the z-score threshold (verified against `_positions_core`).
* The Methodology section overclaimed that the z-score and the position itself are "computed with `.shift(1)`" → corrected: only the rolling statistics feeding them are (z-score's mean/std, risk_scale's EWMA vol); the causality guarantee that actually matters is that the PnL step lags position and beta by one day, not that every intermediate quantity is itself shifted.
* `main.py`'s walk-forward final-test slice used an open `"2024":"2026"` range, silently including every trading day through whenever the script happens to run, and disagreeing with `oos_evaluation.py`'s `FROZEN_DATE`-pinned window (~1.27 vs. the committed ~1.21 Sharpe on one comparison) → aligned to `FROZEN_DATE`, so `main.py`'s console output for final-test no longer drifts from `results/oos_results.csv`.
* `main.py`'s module docstring said the cost stress test scenarios were "0.5 / 1.0 / 2.0 bps" → corrected to the actual `2.5 / 5.0 / 10.0` bps used in `costs.py` — stale since that scenario-level fix (documented above), never updated in the docstring.
* `market_check.py`'s Brent/WTI contract-count ratio was `wti_contracts * beta`, ignoring the price-level difference between the two contracts entirely → corrected to `wti_contracts * wti_price / (beta * brent_price)`, derived from and cross-checked against a P&L-sensitivity-cancellation argument — see the derivation in `describe_trade_action`'s inline comment, including why beta belongs in the denominator, not multiplied in directly.
* `parameter_sensitivity.py` computed its model directly on the development-period slice rather than an expanding history from the true start of the data → matched to `oos_evaluation.py`'s expanding-history-then-slice pattern. Today's numbers are unaffected (development already starts at the true beginning of the dataset, so there's no earlier history to lose) — this removes a latent fragility for if that ever stops being true, not a current-output change.
* **`oos_evaluation.py`'s `prepare_period` computed `net_log_ret` correctly on the full expanding history, then threw that away and recomputed it by re-invoking `compute_performance` on the already-sliced period.** Re-differencing/re-shifting on a truncated frame has no visible row before the slice's first date, so that first day's return silently became NaN and got dropped — verified empirically: every period's first day (`2019-01-02`, `2022-01-03`, `2024-01-02`) had `net_log_ret = NaN`, exactly one row short of that period's full length. Fixed by aggregating Sharpe/Sortino/return directly from the already-correct, full-history-computed daily series (via `spread_model.performance_metrics_from_log_returns`), sliced only afterward, with `turnover` (for `n_trades`) computed the same way. Development's very first day (`2019-01-02`) stays NaN regardless — there is no earlier data at all before the dataset's true start, which no fix changes — but validation and final-test each recover one valid day. Expected numeric impact is small (one extra day out of 500-750 already used per period) but not exactly zero; re-run `oos_evaluation.py` for the exact refreshed figures.
...
* **`market_check.py` is a research signal check, not a decision-grade tool.** It does not verify exact contract multipliers/tick sizes, currency, margin requirements, data staleness, or roll status before suggesting a position size. Turning it into something safe to act on would need those checks added first. (The Brent/WTI contract ratio itself now accounts for both beta and the price-level ratio between the two contracts — see the derivation in `describe_trade_action` — but everything else in this limitation still applies.)
* **Turnover cost is charged whenever `risk_scale` changes, even on days with no change in the underlying z-score/regime signal.** `risk_scale` moves with estimated portfolio volatility day to day, so a live version of this strategy would either rebalance that often (paying the cost this backtest already charges) or rebalance less often (some tracking-error slippage this backtest doesn't charge). Which is more realistic depends on an operational choice — how often you'd actually rebalance — that this project does not make; the current behavior (rebalance daily to the exact current risk_scale) is one defensible assumption, not verified against the alternative.
...
* **Cost stress test, bid-ask layer, and roll-drag check are all stylized**, not realistic execution/market-impact or futures-curve simulations. The bid-ask layer's tick *size* is verified from real exchange contract specs, but its spread *width* (1-3 ticks) is an assumed range, not measured from real quote data - yfinance provides no bid/ask history. All three would need real order-book and futures-curve data to go further, which this project does not have.
